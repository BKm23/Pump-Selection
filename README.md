...
### Published curve images
Each size record may carry `"curve_image": "data/curves/pNNN.jpg"` — a scan of
the actual GPM9 60 Hz performance-curve page for that size (real efficiency
islands, NPSHr, and power). When a pump is expanded in the UI (Show Curve), the
modelled curve is shown first, then the published image below it for exact-value
verification. Images live in `/data/curves/`. To add more: find the size's
"60 Hz Curves Model …" page in the manual, render it, drop the JPG in
`/data/curves/`, and set `curve_image` on the record.
...
## 3b. Engineering basis & assumptions (full disclosure)

Nothing in the engine is a black box. Every number is one of:

**From the manual (read data):** each size's flow/head coverage window, max
temperature, materials, seals, consistency limits, solids; max impeller (size
name); trim range calibrated from the published 60 Hz curve pages.

**Standard engineering calculations:** `Ns = N√Q/H^0.75`; `Nss = N√Q/NPSHr^0.75`;
affinity laws (`H∝D²`, `Q∝D`, `H∝N²`, `Q∝N`); `WHP = Q·H·SG/3960`; `BHP = WHP/η`;
casing pressure `≥ shutoff/2.31`; `NPSH margin = NPSHa − NPSHr`; NEMA sizing.

**Documented correlations** (the manual gives efficiency/NPSHr only as curve
images, so these come from published engineering correlations, not guesses):
- **Efficiency** — Gülich BEP correlation (Gülich, *Centrifugal Pumps*):
  `η = 1 − 0.095·(1/Q)^0.15 − 0.3·[0.35 − log₁₀(nq/23)]²·(1/Q)^0.05` (nq metric, Q in m³/s).
- **NPSHr(Q)** = `(N·√Q / Nss)^(4/3)`, reference Nss from Hydraulic Institute
  ranges: 9000 single-suction, 11000 double-suction, 6000 MC, 12000 axial.
- **Viscosity** — ANSI/HI 9.6.7 parametric approximation.

**Remaining standard-value assumptions** (documented in `tools`/build script,
used only to shape the modelled curve — read exact values from the published
image): BEP at 80% of max flow; end-of-curve head ≈ 35% of shutoff; part-load
efficiency parabolic about the Gülich BEP; min continuous flow ≈ 15% of max.
Scoring weights in `config.js` are ranking policy, not physics.
