...
(function (root) {
  ...
  function warnings(pump, s, inp, curve) {
    ...
    else if (pump.data_quality === "coverage_chart") out.push("Size from GPM9 coverage chart — per-size curve modelled; verify efficiency/NPSHr vs Goulds PSS");
  ...
  /* ---------------- "Why no match?" diagnostic ---------------------------
     When selectPumps returns nothing, this explains WHY in plain language:
     it looks at the most relevant pump family for the fluid, finds the
     closest-miss pumps, and reports the specific blocking reasons plus one
     actionable hint. Returns { message, closest:[...], hint }.              */
  function diagnoseNoMatch(inp, vendorDatasets, cfg) {
    const all = selectPumps(inp, vendorDatasets, cfg, { keepRejected: true });
    const rejected = all.filter(r => r.rejected);
    if (!rejected.length)
      return { message: "No pump data is loaded for the selected vendors.", closest: [], hint: "" };

    // Narrow to the family that suits this fluid, if one is defined.
    const fp = fluidProfile(inp.fluid);
    const fam = fp.restrictTo || fp.ideal || null;
    let rel = rejected;
    if (fam) {
      const inFam = rejected.filter(r => (r.pump.service_category || []).some(c => fam.includes(c)));
      if (inFam.length) rel = inFam;
    }
    // Closest misses first (fewest blocking reasons).
    rel.sort((a, b) => a.rejected.length - b.rejected.length);

    // Tally reasons (numbers stripped so like reasons group together).
    const tally = {};
    rel.forEach(r => r.rejected.forEach(x => {
      const k = x.replace(/-?\d+(\.\d+)?/g, "#");
      tally[k] = (tally[k] || 0) + 1;
    }));
    const topReason = Object.keys(tally).sort((a, b) => tally[b] - tally[a])[0] || "";

    // One actionable hint based on the dominant blocker.
    let hint = "";
    if (/flow below min/.test(topReason))
      hint = "The duty flow is below the minimum for this pump family. Increase flow, reduce speed, or use a smaller pump size / recessed-impeller pump.";
    else if (/below MC pump minimum/.test(topReason))
      hint = "This consistency is below the MC pump's 8% minimum. Standard stock pumps suit lower consistency — but they are higher-capacity, so a low flow at low consistency may fall outside the Goulds stock range.";
    else if (/above pump limit/.test(topReason))
      hint = "The stock consistency exceeds the standard stock pumps' limit; an MC pump (3500XD, 8-16%) is needed, but check the flow is within its 220-2200 GPM range.";
    else if (/flow above max/.test(topReason))
      hint = "The duty flow exceeds the maximum for this family. Consider a higher-capacity pump or split the duty across parallel pumps.";
    else if (/head not achievable/.test(topReason))
      hint = "The required head exceeds what these pumps make at this flow. Consider a multi-stage pump or a higher-speed selection.";
    else if (/over temperature/.test(topReason))
      hint = "Duty temperature exceeds the pumps' rating; a high-temperature model is required.";
    else if (/over pressure/.test(topReason))
      hint = "Duty pressure exceeds the pumps' casing rating; a higher-pressure class is required.";
    else if (/material/.test(topReason))
      hint = "No loaded pump offers a suitable material for this fluid.";

    const closest = rel.slice(0, 3).map(r => ({
      model: r.pump.vendor + " " + r.pump.model,
      reasons: r.rejected
    }));
    const cons = dutyConsistency(inp);
    let where = Math.round(inp.flowGpm) + " gpm @ " + Math.round(inp.tdhFt) + " ft";
    if (cons != null) where += ", " + cons + "% consistency";
    const famWord = fam ? (inp.fluid + " ") : "";
    const message = "No " + famWord + "pump matches this duty (" + where + ").";
    return { message, closest, hint };
  }
  ...
  const API = {
    ...
    selectPumps, diagnoseNoMatch, recommendMaterial, clamp
  };
  root.PumpEngine = API;
