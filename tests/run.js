...
console.log("\n[9] 'Why no match?' diagnostic (40 gpm / 300 ft paper stock — below smallest size)");
const noMatchDuty = Object.assign({}, base, {fluid:"paper stock", flowGpm:40, tdhFt:300, solidsPct:4});
const nm = E.selectPumps(noMatchDuty, [goulds], cfgShim(), {topN:5});
ok("selection is empty at 40 gpm / 300 ft (below smallest stock size)", nm.length===0, nm.length);
const dx = E.diagnoseNoMatch(noMatchDuty, [goulds], cfgShim());
console.log("    message:", dx.message);
console.log("    hint   :", dx.hint);
dx.closest.forEach(c=>console.log("    -", c.model, "=>", c.reasons.join("; ")));
ok("diagnostic returns a message", !!dx.message && /paper stock/.test(dx.message));
ok("diagnostic lists closest excluded stock pumps", dx.closest.length>0);
ok("closest pumps are all Pulp & Paper family",
   dx.closest.every(c => /3175|3180|3185|3181|3186|3500XD/.test(c.model)),
   dx.closest.map(c=>c.model).join(","));
ok("diagnostic gives an actionable hint", !!dx.hint && dx.hint.length>10);

console.log("\n[10] Per-size stock selection (3175 expanded to individual sizes)");
const psz = Object.assign({}, base, {fluid:"paper stock", flowGpm:700, tdhFt:100, solidsPct:4});
const pszRes = E.selectPumps(psz, [goulds], cfgShim(), {topN:5});
console.log("    700 gpm/100 ft ->", pszRes.map(r=>r.pump.model+" "+r.pump.size).join(", "));
const STOCK_MODELS = ["3175","3180 / 3185","3181 / 3186","3500XD"];
ok("700 gpm now selects a specific stock size (not 'no match')",
   pszRes.length>0 && STOCK_MODELS.includes(pszRes[0].pump.model) && /\d+x\d+-\d+/.test(pszRes[0].pump.size),
   pszRes.length?pszRes[0].pump.model+" "+pszRes[0].pump.size:"(none)");
ok("every 700 gpm pick is a real size configuration",
   pszRes.every(r => /\d+x\d+-\d+/.test(r.pump.size)));
ok("small duty (400 gpm) selects a small 3x6 size",
   /^3x6-/.test((E.selectPumps(Object.assign({},base,{fluid:"paper stock",flowGpm:400,tdhFt:90,solidsPct:4}),[goulds],cfgShim(),{topN:1})[0]||{pump:{}}).pump.size||""),
   (E.selectPumps(Object.assign({},base,{fluid:"paper stock",flowGpm:400,tdhFt:90,solidsPct:4}),[goulds],cfgShim(),{topN:1})[0]||{pump:{size:"none"}}).pump.size);
ok("12% MC duty selects the 3500XD MC pump",
   (E.selectPumps(Object.assign({},base,{fluid:"paper stock",flowGpm:1500,tdhFt:400,solidsPct:12}),[goulds],cfgShim(),{topN:1})[0]||{pump:{}}).pump.model === "3500XD",
   (E.selectPumps(Object.assign({},base,{fluid:"paper stock",flowGpm:1500,tdhFt:400,solidsPct:12}),[goulds],cfgShim(),{topN:1})[0]||{pump:{model:"none"}}).pump.model);
ok("each stock model now has multiple size records",
   STOCK_MODELS.every(m => goulds.pumps.filter(p=>p.model===m).length >= 5),
   STOCK_MODELS.map(m=>m+":"+goulds.pumps.filter(p=>p.model===m).length).join(", "));
